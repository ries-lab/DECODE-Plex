...
class FrameMapTensor(
    slicing._LinearGetitemMixin, slicing._SizebyFirstMixin, tensor.TensorMemoryMapped
):
    def __init__(self, paths: Sequence[Path], loader: Callable[[Path], torch.Tensor]):
        """
        Construct memory mapped tensor from path to tiff file

        Args:
            path:
            loader: callable to load an individual file
        """
        self._paths = paths
        self._loader = loader
    ...
    def _get_element(self, item: int) -> torch.Tensor:
        return self._loader(self._paths[item])
...
class TiffTensor(
    ...
    def _get_element(self, item) -> torch.Tensor:
...
def auto_discover_ome(
    path: Path, pattern_ome: str = ".ome", pattern_suffix: str = "_[0-9]{1,2}"
) -> list[Path]:
    """
    ...
    Returns:
        list of paths
    """
    base = path.stem.strip(pattern_ome)

    files = sorted(path.parent.glob(f"{base}*{path.suffix}"))
