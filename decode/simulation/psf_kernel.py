...
class PSF(ABC):
    ...
    def forward(
        ...
        Implementation methods should call this method first in order not to handle the
        default argument stuff.

        Example::
            If implementation do not implement a batched forward method, they may call
            this method, and then refer to the single-frame wrapper as.
            Their forward implementation will then look like this:
    ...
    def _auto_filter_shift(
        ...
        # kick out everything that is out of frame index bounds
        # and shift the frames to start at 0
...
class DeltaPSF(PSF):
    ...
    def search_bin_index(self, xy: torch.Tensor, raise_outside: bool = True):
        ...
        or handle those items later on.

        Args:
            xy: xy coordinates
            raise_outside: raise error if anything is outside of the specified bins;
             otherwise those coordinate's indices are -1 and len(bin) which means
             outside of the bin's range.
    ...
    def forward(
        ...
        mask = self._fov_filter._kernel.mask(xyz)
...
class _WashedDeltaPSF(PSF):
    def __init__(
        self,
        xextent: tuple[float, float],
        yextent: tuple[float, float],
        img_shape: tuple[int, int],
        weight_lr: tuple[float, float] = (1., 1.)
    ):
        """
        Mainly for testing purposes. This PSF puts a weight corresponding to the
        offset in the pixel next to it.

        ToDo: Possibly add top, bottom kernels.

        Args:
            xextent:
            yextent:
            img_shape:
            weight_lr: weight factor to apply to the left and right pixel
        """
        super().__init__(xextent, yextent, img_shape=img_shape)

        self._weight_lr = weight_lr

        # check that pixel size is 1
        if (xextent[1] - xextent[0]) != img_shape[0] or (yextent[1] - yextent[0]) != img_shape[1]:
            raise ValueError("Pixel size must be 1.")

        self._kernel_ctr = DeltaPSF(xextent, yextent, img_shape)
        self._kernel_right = DeltaPSF(xextent, yextent, img_shape)
        self._kernel_left = DeltaPSF(xextent, yextent, img_shape)
    ...
    def forward(
        ...
        frame_ctr = self._kernel_ctr.forward(xyz, weight, frame_ix, ix_low, ix_high)

        # weight to the left/right is offset to center to the left/right
        xyz_left = xyz.clone()
        xyz_left[:, 0] -= 1
        weight_left = (1 - xyz_left[:, 0] % 1).clip(min=0)
        weight_left[weight_left > 0.5] = 0
        weight_left *= self._weight_lr[0]
        frame_left = self._kernel_left.forward(
            xyz_left, weight_left, frame_ix, ix_low, ix_high
        )

        xyz_right = xyz.clone()
        xyz_right[:, 0] += 1
        weight_right = (xyz_right[:, 0] % 1).clip(min=0)
        weight_right[weight_right >= 0.5] = 0
        weight_right *= self._weight_lr[1]
        frame_right = self._kernel_right.forward(
            xyz_right, weight_right, frame_ix, ix_low, ix_high
        )

        return frame_ctr + frame_left + frame_right
...
class GaussianPSF(PSF):
    ...
    def _forward_single_frame(self, xyz: torch.Tensor, weight: torch.Tensor):
        ...
        if self.peak_weight:
            # ToDo: broken, check whats wrong.
            raise NotImplementedError(
                "Something wrong with the implementation."
                "Not supported in this version of decode."
            )
...
class CubicSplinePSF(PSF):
    ...
    def __init__(
        ...
        *,
        ...
        flip_x: bool = False,
        flip_y: bool = False,
        glob_shift: list | torch.Tensor = None,
        device: str = "cpu",
        max_roi_chunk: int = 500000,
        max_frame_chunk: int = 100000,
    ):
        """
        ...
        Args:
            ...
            ref_re: redefined reference point
            norm: normalization factor if the coefficients are not such that the ROI is
                normalized to 1.
            roi_auto_center:
            flip_x: flip roi-wise along x axis before frame aggregation
            flip_y: flip roi-wise along y axis before frame aggregation
            glob_shift: global shift of the PSF in px units, useful to correct for
                flipping with non-centered PSFs
            ...
            max_frame_chunk (int): max number of frames to be processed at a time via
                the cuda kernel. If you run into memory allocation errors, decrease this
                number or free some space on your CUDA device.
        """
        super().__init__(
        ...
        self.norm = norm if norm is not None else 1.0

        self._device = (
            torch.device(device) if not isinstance(device, torch.device) else device
        )
        self.max_roi_chunk = max_roi_chunk
        self.max_frame_chunk = max_frame_chunk
        self._flip_x = flip_x
        self._flip_y = flip_y
        self._glob_shift = (
            torch.as_tensor(glob_shift).to(self._device)
            if glob_shift is not None
            else None
        )
    ...
    def _init_spline_impl(self):
        """
        Init the spline implementation, done separately because otherwise
        it's harder to pickle
        """
    ...
    def sanity_check(self):
        ...
        if self._flip_x or self._flip_y:
            logger.warn("Flipping PSF ROI-wise is experimental.")
    ...
    def cuda_compiled(self) -> bool:
        """
        Returns true if (1) a CUDA capable device is available and
        (2) spline was compiled with CUDA support.
    ...
    def _ref_diff(self):
        if self.ref_re is None:
            return torch.zeros((3), device=self.ref0.device)
        else:
            return self.ref_re.to(self.ref0.device) - self.ref0
    ...
    def __getstate__(self):
        """
        Returns dict without spline implementation attribute
        because C++ / CUDA implementation is not (yet) implemented to be pickleable
        itself. However, since the CUDA/C++ implementation is only  accessed by this
        wrapper, this is not strictly needed. This class becomes pickleable by excluding
        the spline implementation and rather re-init the implementation every time it is
        unpickled.
    ...
    def to(self, device: Union[str, torch.device]) -> "CubicSplinePSF":
        """
        Returns psf instance on specified device.
        ...
        if device.type == "cuda":
            return self.cuda(device.index if device.index is not None else 0)
        elif device.type == "cpu":
            return self.cpu()

    def cuda(self, ix: int = 0):
        """
        Returns a copy of this object with implementation in CUDA.
        If already on CUDA and selected device, return original object.
        ...
        """
        if self._device.type == "cuda" and self._device.index == ix:
            return self
        ...
        return CubicSplinePSF(
            ...
            flip_x=self._flip_x,
            flip_y=self._flip_y,
            glob_shift=self._glob_shift,
        )

    def cpu(self):
        """
        Returns a copy of this object with implementation in CPU code.
        If already on CPU, return original object.
    ...
    def phot_norm(self, phot: torch.Tensor) -> torch.Tensor:
        """
        Normalise photons, useful for PSFs that are not normalized to one.
        This function must be used before any ROI, Frame or Derivative calculation.

        Args:
            phot:

        """
        return phot / self.norm

    def pos2pos_rel(self, xyz: torch.Tensor) -> torch.Tensor:
        """
        Convert absolute positions to relative positions
        (i.e. relative to the x/y extents).
        Should only be used in frame based settings, not for ROIs.

        Args:
            xyz: coordinates of size N x (2 or 3)
        """

        # + 0.5 because 0 is at center; needs scale to be 1
        xyz[..., 0] = xyz[..., 0] - (self.xextent[0] + 0.5)
        xyz[..., 1] = xyz[..., 1] - (self.yextent[0] + 0.5)

        return xyz

    def coord2impl(self, xyz: torch.Tensor) -> torch.Tensor:
        """
        Transforms nanometre coordinates to implementation coordiantes.
        Does not respect shifts as by extent.
        ...
        self.vx_size = self.vx_size.to(xyz.device)
        self.ref0 = self.ref0.to(xyz.device)

        # left for legacy info
        offset = torch.zeros(3, dtype=torch.float, device=xyz.device)
        return -xyz / self.vx_size + offset + self.ref0 - self._ref_diff

    def frame2roi_coord(self, xyz_nm: torch.Tensor):
        """
        Computes ROI wise coordinate from the coordinate on the frame and returns the
        px on the frame in addition
        ...
        Returns:
            ...
            onframe_ix: ix where to place the roi on the final frame (caution: might
            be negative when roi is outside
            frame, but then we don't mean negative in pythonic sense)

        """
        xyz_r = xyz_nm.clone()

        self.vx_size = self.vx_size.to(xyz_nm.device)
        self.ref0 = self.ref0.to(xyz_nm.device)
    ...
    def forward_rois(self, xyz, phot):
        ...
        Returns:
            torch.Tensor with size N x roi_x x roi_y where N is the number of
            emitters / coordinates and roi_x/y the respective ROI size

        """

        phot = self.phot_norm(phot)
    ...
    def _forward_frame_chunks(
        ...
        # splits forward into chunks and collates afterwards

        ix_low_chunk = ix_low
        f = []
        while ix_low_chunk < ix_high:
            ix_high_chunk = min(ix_low_chunk + chunk_size, ix_high)

            in_range = (ix_low_chunk <= frame_ix) & (frame_ix < ix_high_chunk)

            f.append(
                self.forward(
                    xyz=xyz[in_range],
                    weight=weight[in_range],
                    frame_ix=frame_ix[in_range],
                    ix_low=ix_low_chunk,
                    ix_high=ix_high_chunk,
                )
            )
            ix_low_chunk += chunk_size

        return torch.cat(f)

    def _forward_roi_chunks(
        self,
        xyz: torch.Tensor,
        weight: torch.Tensor,
        frame_ix: torch.Tensor,
    ...
    def forward(
        ...
        # recursions to prevent CUDA out of memory errors
        if (
            self.max_frame_chunk is not None
            and (ix_high - ix_low) > self.max_frame_chunk
        ):
            return self._forward_frame_chunks(
                xyz, weight, frame_ix, ix_low, ix_high, self.max_frame_chunk
            )

        if self.max_roi_chunk is not None and len(xyz) > self.max_roi_chunk:
            return self._forward_roi_chunks(
                xyz, weight, frame_ix, ix_low, ix_high, self.max_roi_chunk
            )

        # flipping
        flip_dict = {0: self._flip_x, 1: self._flip_y}
        for ch, flip in flip_dict.items():
            if flip:
                mask = xyz[:, ch] % 1 != 0
                x = xyz[mask, ch].floor() + 2 - (xyz[mask, ch] % 1)
                xyz[mask, ch] = x

        if self._glob_shift is not None:
            xyz = xyz + self._glob_shift

        weight = self.phot_norm(weight)

        # shift coordinates in reference frame defined by xextent/yextent and
        # convert coordinates into ROI based coordinates and transform
        # into implementation coordinates
        xyz = self.pos2pos_rel(xyz)
...
class ZernikePSF(PSF):
    @deploy.raise_optional_deps(
        "zernike", "please install zernike via `pip install zernike`"
    )
    def __init__(
        self,
        xextent,
    ...
    def _get_kxyz(self):
        ...
        krange = np.linspace(
            -self._pupil_radius + self._pupil_radius / self._k_size,
            self._pupil_radius - self._pupil_radius / self._k_size,
            self._k_size,
        )
        ...
        kz = np.lib.scimath.sqrt(
            (self._ref_index / self._wavelength) ** 2
            - (kr * self._num_aperture / self._wavelength) ** 2
        )

        return kx, ky, kz

    def _get_unit_vectors(self):
        kx, ky = torch.linspace(-1, 1, self._k_size), torch.linspace(
            -1, 1, self._k_size
        )
    ...
    def _forward_single_frame(self, xyz: torch.Tensor, weight: torch.Tensor):
        ...
        h_k = (
            h
            * self._e_mn
            * torch.exp(2j * math.pi * (self._kx * x + self._ky * y + self._kz * z))
        )
