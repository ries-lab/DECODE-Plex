...
class Gamma(NoiseDistribution):
    ...
    def forward(self, x):
        ...
        return torch.distributions.gamma.Gamma(
            x, 1 / self.scale, validate_args=False
        ).sample()
...
class Poisson(NoiseDistribution):
    """
    Poisson noise. 'Non-parametric' with respect  initialisation since the only
    parameter (lambda) comes from the input in the forward method itself.
    """
