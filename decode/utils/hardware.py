...
def _specific_device_by_str(device: str) -> Tuple[str, Optional[int]]:
    """
...
def collect_hardware() -> dict:
    """Collects general hardware information for logging"""

    cuda = []
    for ix in range(torch.cuda.device_count()):
        properties = torch.cuda.get_device_properties(ix)
        cuda.append({k: getattr(properties, k) for k in dir(properties) if k[0] != "_"})
...
def get_max_batch_size(
    ...
    if device == torch.device("cpu"):
        raise ValueError(
            f"Determining the max batch size makes only "
            f"sense when running on an accelerated device. Got {device}"
        )
    ...
    while bs < size_high:
        ...
        if bs_pass is None:
            raise RuntimeError(
                "Lowest possible batch size is outside of provided bounds."
            )
