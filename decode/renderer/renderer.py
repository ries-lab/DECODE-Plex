...
class Renderer(ABC):
    def __init__(
        ...
        """Renderer; tkes emitters and outputs a rendered image."""
        super().__init__()
...
class Renderer2D(Renderer):
    def __init__(
        ...
        self._cmap = cmap
        self.jet_hue = self._get_jet_cmap(cmap)
    ...
    def forward(self, em: EmitterSet, col_vec=None) -> torch.Tensor:
        ...
        if col_vec is not None:
            ...
            self.colextent = (
                (col_vec.min(), col_vec.max())
                if self.colextent is None
                else self.colextent
            )
            int_hist, col_hist = self._hist2d(
                em_sub,
                col_vec,
                xyz_extent[self.plot_axis[0]],
            ...
            if self.rel_clip is not None:
                int_hist = np.clip(
                    int_hist * self.contrast, 0.0, int_hist.max() * self.rel_clip
                )
    ...
    def get_extent(self, em) -> Tuple[tuple, tuple, tuple]:
        xextent = (
            (em.xyz_nm[:, 0].min(), em.xyz_nm[:, 0].max())
            if self.xextent is None
            else self.xextent
        )
        yextent = (
            (em.xyz_nm[:, 1].min(), em.xyz_nm[:, 1].max())
            if self.yextent is None
            else self.yextent
        )
        zextent = (
            (em.xyz_nm[:, 2].min(), em.xyz_nm[:, 2].max())
    ...
    def _hist2d(self, em: EmitterSet, col_vec, x_hist_ext, y_hist_ext, c_range=None):
        ...
        hist_bins_x = np.arange(
            x_hist_ext[0], x_hist_ext[1] + self.px_size, self.px_size
        )
        hist_bins_y = np.arange(
            y_hist_ext[0], y_hist_ext[1] + self.px_size, self.px_size
        )

        int_hist, _, _ = np.histogram2d(
            xy[:, 0], xy[:, 1], bins=(hist_bins_x, hist_bins_y)
        )
        ...
        if col_vec is not None:
            ...
            col_hist, _, _ = np.histogram2d(
                xy[:, 0], xy[:, 1], bins=(hist_bins_x, hist_bins_y), weights=c_weight
            )
    ...
    @staticmethod
    def _get_jet_cmap(name: str):
        lin_hue = np.linspace(0, 1, 256)
        cmap = plt.get_cmap(name, lut=256)
        cmap = cmap(lin_hue)
        ...
        jet_hue = np.interp(
            np.linspace(0, len(jet_hue), 256), np.arange(len(jet_hue)), jet_hue
        )
        return jet_hue
...
class RendererIndividual2D(Renderer2D):
    ...
    def _place_gaussians_weighted(comb_hist, inds, weights, W, fs):
        for i in range(len(W)):
            comb_hist[
                inds[i, 1] : inds[i, 1] + fs, inds[i, 0] : inds[i, 0] + fs
    ...
    def _hist2d(self, em: EmitterSet, col_vec, x_hist_ext, y_hist_ext, c_range=None):
        ...
        s_inds = (
            (xy_mus - torch.Tensor([x_hist_ext[0], y_hist_ext[0]]).to(self.device))
            // self.px_size
        ).type(torch.LongTensor)

        if col_vec is not None:
            c_pos = torch.clip(col_vec, c_range[0], c_range[1])
            c_weight = ((c_pos - c_range[0]) / (c_range[1] - c_range[0])).to(
                self.device
            )

            comb_hist = torch.zeros(
                [h + self.fs, w + self.fs, 2], device=self.device, dtype=torch.float
            )
            ...
            for i in tqdm(range(len(xy_mus) // self.bs + 1)):
                ...
                c_ws = c_weight[sl]
                comb_hist = self._place_gaussians_weighted(
                    comb_hist, sub_inds, c_ws, W, torch.tensor(self.fs)
                )

            comb_hist = comb_hist[
                self.fs // 2 : -(self.fs // 2 + 1), self.fs // 2 : -(self.fs // 2 + 1)
            ]
            ...
            int_hist = torch.zeros(
                [h + self.fs, w + self.fs], device=self.device, dtype=torch.float
            )
            ...
            for i in tqdm(range(len(xy_mus) // self.bs + 1)):
                ...
                sub_inds = s_inds[sl]
                W = self.calc_gaussians(xy_mus[sl], xy_sigs[sl], mesh)
                int_hist = self._place_gaussians(
                    int_hist, sub_inds, W, torch.tensor(self.fs)
                )
