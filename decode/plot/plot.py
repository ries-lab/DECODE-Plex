...
class PlotFrame:
    ...
    def plot(self) -> plt.axis:
        ...
        if self.extent is None:
            im = plt.imshow(self.frame.numpy(), cmap="gray")
        else:
            im = plt.imshow(
                self.frame.numpy(),
                cmap="gray",
    ...
    def plot(self):
        ...
        if self.pos_tar is not None:
            if self.phot_tar is not None:
                ...
                plot_xyz(
                    self.pos_tar, self.tar_marker[1], self.tar_marker[0], self.labels[0]
                )
        ...
        if self.pos_out is not None:
            if self.phot_out is not None:
                ...
                plot_xyz(
                    self.pos_out, self.out_marker[1], self.out_marker[0], self.labels[1]
                )
        ...
        if self.pos_ini is not None:
            if self.phot_ini is not None:
                ...
                plot_xyz(
                    self.pos_ini, self.ini_marker[1], self.ini_marker[0], self.labels[2]
                )
...
class PlotCoordinates3D:
    ...
    def __init__(
        self, pos_tar=None, pos_out=None, phot_out=None, match_lines=False, labels=None
    ):
        self.pos_tar = pos_tar
        self.pos_out = pos_out
        self.phot_out = phot_out
