...
class EmitterMatcher(ABC):
    _return_match = namedtuple(
        "MatchResult", ["tp", "fp", "fn", "tp_match"]
    )  # return-type as namedtuple
    ...
    @abstractmethod
    def forward(
        self, output: emitter.EmitterSet, target: emitter.EmitterSet
    ) -> _return_match:
        """
...
class GreedyHungarianMatching(EmitterMatcher):
    def __init__(
        ...
        *,
        ...
        if (
            self.dist_lat is not None
            and self.dist_ax is not None
            and self.dist_vol is not None
        ):
            warnings.warn(
                "You specified a lateral, axial and volumetric threshold. "
                "While this is allowed; are you sure?"
            )

        if self.dist_lat is None and self.dist_ax is None and self.dist_vol is None:
            warnings.warn(
                "You specified neither a lateral, axial nor volumetric threshold. Are you sure about this?"
            )
    ...
    def filter(self, xyz_out: torch.Tensor, xyz_tar: torch.Tensor) -> torch.Tensor:
        ...
        filter_mask = torch.ones(
            (xyz_out.size(0), xyz_out.size(1), xyz_tar.size(1))
        ).bool()
    ...
    def _match_kernel(self, xyz_out, xyz_tar, filter_mask):
        ...
        if self.match_dims == 2:
            dist_mat = torch.cdist(
                xyz_out[None, :, :2], xyz_tar[None, :, :2], p=2
            ).squeeze(0)

        elif self.match_dims == 3:
            dist_mat = torch.cdist(
                xyz_out[None, :, :], xyz_tar[None, :, :], p=2
            ).squeeze(0)
    ...
    def forward(self, output: emitter.EmitterSet, target: emitter.EmitterSet):
        ...
        if len(output) >= 1 and len(target) >= 1:
            frame_low = (
                ...
                else target.frame_ix.min()
            )
            frame_high = (
                output.frame_ix.max()
                if output.frame_ix.max() > target.frame_ix.max()
                else target.frame_ix.max()
            )
        elif len(output) >= 1:
        ...
        for out_f, tar_f in zip(out_pframe, tar_pframe):
            ...
            tp_ix, tp_match_ix, tp_ix_bool, tp_match_ix_bool = self._match_kernel(
                out_f.xyz_nm, tar_f.xyz_nm, filter_mask
            )  # non batch impl.
