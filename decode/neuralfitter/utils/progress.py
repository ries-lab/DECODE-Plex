...
class GMMHeuristicCheck(ProgressCheck):
    def __init__(
        self, emitter_avg: float, threshold: float = 100.0, ref_epoch: int = 1
    ):
        """
    ...
    def check_progress(self, gmm_loss: float | torch.Tensor, epoch: int) -> bool:
        if (
            self._prev_converged is not None
        ):  # do not check if checked before in ref epoch
