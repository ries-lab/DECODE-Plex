...
class ParameterList:
    ...
    def forward(self, em: EmitterSet) -> tuple[torch.Tensor, torch.BoolTensor]:
        ...
        # number of channels as by phot, note: this is mutually exclusive with codes
        n_ch = em.phot.size(-1) if em.phot.dim() == 2 else 1
        n_tar = 3 + n_ch

        # compute target (i.e. a matrix / tensor in which all params are concatenated)
        tar = torch.ones((self._n_frames, self._n_em, n_tar)) * float("nan")
        mask = torch.zeros(
            (self._n_frames, self._n_em, self._n_codes), dtype=torch.bool
        )

        em_split = em.split_in_frames(0, self._n_frames)

        # set number of active elements per frame
        for i, em_frame in enumerate(em_split):  # range(self._n_frames):
            ...
            tar[i, :n_emitter, :n_ch] = em_frame.phot.view(-1, n_ch)
            tar[i, :n_emitter, n_ch:] = getattr(em_frame, f"xyz_{self._xy_unit}")
...
@deprecated(
    reason="used in previous versions of decode; for reference only", version="0.11"
)
...
@deprecated(reason="not used", version="0.11", action="error")
