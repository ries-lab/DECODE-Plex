...
class ToEmitterLookUpPixelwise(ToEmitter):
    ...
    def forward(self, x: torch.Tensor) -> emitter.EmitterSet:
        ...
        # max out code_dim if not reduced implicitly by squeeze before
        features["prob"] = (
            features["prob"].max(-1)[0]
            if features["prob"].dim() > 1
            else features["prob"]
        )
...
class ToEmitterSpatialIntegration(ToEmitterLookUpPixelwise):
    def __init__(
        ...
        Args:
            mask: probability threshold from where detections are considered
    ...
    def forward(self, x: torch.Tensor) -> emitter.EmitterSet:
        x[:, self._ch_map.ix_prob] = self._non_max_suppression(
            x[:, self._ch_map.ix_prob]
        )
    ...
    def _non_max_suppression(self, p: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            filt = (
                torch.tensor(
                    [[diag, 1.0, diag], [1, 1, 1], [diag, 1, diag]], device=p.device
                )
                .unsqueeze(0)
                .unsqueeze(0)
            )
            ...
            conv = torch.nn.functional.conv2d(
                p.view(-1, 1, *p.shape[-2:]), filt, padding=1
            ).view(p.shape)
