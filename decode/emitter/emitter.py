...
class EmitterData(models.MixinEqualLength, models.MixinEqualDevice, BaseModel):
    """
    ...
    phot: models.Tensor
    frame_ix: models.LongTensor

    id: Optional[models.LongTensor]
    code: Optional[models.LongTensor]
    prob: Optional[models.Tensor]
    prob_code: Optional[models.Tensor]
    bg: Optional[models.Tensor]

    xyz_cr: Optional[models.Coordinate]
    phot_cr: Optional[models.Tensor]
    bg_cr: Optional[models.Tensor]

    xyz_sig: Optional[models.Coordinate]
...
class EmitterSet:
    ...
    Attributes:
            ...
            xyz_px: Coordinates in pixel units.
            xyz_nm: Coordinates in nanometer units.
            phot: Photon count of size N
            frame_ix: size N. Index on which the emitter appears.
            id: size N. Identity of the emitter.
            code: size N.
    ...
    def __init__(
        ...
        *,
        ...
        prob_code: Optional[torch.Tensor] = None,
        ...
        Args:
            ...
            prob_code: Probabilistic code of the emitter. Size :math:`(N,M)`, where M is the number of codes.
        ...
        """
        # use pydantic data container if sanity_check is true else dataclass
        self._data_container = EmitterData if sanity_check else EmitterData.construct
        ...
        self.px_size = torch.as_tensor(px_size) if px_size is not None else None
    ...
    @property
    def xyz_sig_lat_nm(self) -> torch.Tensor:
        return (
            (self.xyz_sig_nm[..., :2] ** 2).sum(-1).sqrt()
            if self.xyz_sig_nm is not None
            else None
        )

    @property
    def xyz_sig_ax_nm(self) -> torch.Tensor:
        return self.xyz_sig_nm[..., 2] if self.xyz_sig_nm is not None else None
    ...
    @property
    def xyz_sig_vol_nm(self) -> torch.Tensor:
        # alias
        return self.xyz_sig_tot_nm
    ...
    def data(self) -> dict:
        ...
        return {k: getattr(self, k) for k in self._data_holder}
    ...
    def sanity_check(self):
        if issubclass(self._data_container, EmitterData):
            *_, validation_error = validate_model(
                self._data.__class__, self._data.__dict__
            )

            if validation_error:
                raise validation_error

        _ = self._get_subset(slice(None), sanity_check=True)
    ...
    def __str__(self):
        ...
        if len(self) >= 1:
            ...
            print_str += (
                ...
                f"\n::spanned volume: {self.xyz.min(0)[0].tolist()} - {self.xyz.max(0)[0].tolist()}"
                f"\n::photon range: {self.phot.min().item()} - {self.phot.max().item()}"
            )
        return print_str
    ...
    def to(self, d: Union[str, torch.dtype, torch.device]):
        em = self.clone()
        # use keys of dictionary data to get those properties into this method
        for k, v in em.data.items():  # .items returns kv pair
            # set attirbute by variable
            if v is not None:
                setattr(em, k, v.to(d))  # sends to device
        return em
    ...
    def cat(
        ...
        Args:
            ...
            sanity_check: perform sanity check on concatenated emitters, default is
             False because we assume individual EmitterSets to have been already
             checked.
        ...
        return cls(xy_unit=xy_unit, px_size=px_size, sanity_check=sanity_check, **data)
    ...
    def _get_subset(self, ix: Union[int, slice], sanity_check: bool = False):
        """
        ...
        Args:
            ...
            sanity_check: perform sanity check on subset, disabled by default for speed
    ...
    def groupby(
        ...
        groupby_window: int | None = None,
        aggregate_fn: Callable = torch.nansum,
        aggregate_fields: list[str] | tuple[str] = ("phot", "bg"),
        sanity_check: bool = False,
    ) -> "EmitterSet":
        """
        Returns EmitterSet where all emitters in the same group are aggregated on the specified fields.

        Args:
            groupby: field name(s) to group by
            weight: importance weight for aggregation (multiply aggregated fields by weight)
            normalize: normalize aggregated fields by weight sum (note: you probably do not want to use this with torch.nanmean)
            groupby_window: window size for temporal context
            aggregate_fn: function to aggregate aggregate_fields of emitters in the same group
               (must ignore nans, e.g. torch.nansum, torch.nanmean, numpy.nanmin, numpy.nanmax)
            aggregate_fields: fields to aggregate with aggregate_fn, all others are copied as they are
            sanity_check: perform sanity check on subset, disabled by default for speed

        Returns:
            EmitterSet where all emitters in the same group are aggregated on the specified fields
        """
        # groupby to list
        if isinstance(groupby, str):
            groupby = [groupby]
        # check fields
        if set(groupby) - set(self.data.keys()):
            raise ValueError(f"Can not group by field(s) {groupby}.")
        if groupby_window is not None and (
            groupby_window < 1 or groupby_window % 2 == 0
        ):
            raise ValueError("If specified, groupby_window must be >= 1 and odd.")
        if normalize and weight is None:
            raise ValueError("If normalize is True, weight must be specified.")

        grouped_data = copy.deepcopy(self.data)
        # sort data by groupby fields and frame_ix
        comp_data = torch.stack(
            [self.data[k] for k in groupby + ["frame_ix"]] + [torch.arange(len(self))],
            dim=-1,
        ).tolist()
        sorted_data = torch.tensor(sorted(comp_data))
        sorted_idxs = sorted_data[:, -1]

        # windowed grouping data: for compute/memory, only consider small window
        # where other relevant emitters might be found
        sorted_grouping_data = sorted_data[:, : len(groupby) + 1]
        windowed_grouping_data = []
        if groupby_window:
            # if >1 emitters of the same group are present per frame:
            # on each side can have at most:
            #  - (groupby_window // 2) + 1 different frames for same emitter
            #  - max_same_em_per_frame emitters of the same group per frame
            # need to adapt window size to include those too
            max_same_em_per_frame = np.unique(
                sorted_grouping_data, axis=0, return_counts=True
            )[1].max()
            window_side = ((groupby_window // 2) + 1) * max_same_em_per_frame - 1
        else:
            # make sure all grouping emitters are present in window
            # by using maximum group size
            max_same_em = np.unique(
                sorted_data[:, : len(groupby)], axis=0, return_counts=True
            )[1].max()
            window_side = max_same_em - 1
        offsets = list(range(-window_side, window_side + 1))
        windowed_grouping_data = torch.stack(
            [torch.roll(sorted_grouping_data, d, dims=0) for d in offsets], dim=0
        )

        # check if all groupby fields are equal
        masks = (windowed_grouping_data[:, :, :-1] == sorted_grouping_data[:, :-1]).all(
            dim=-1
        )
        # chek if within groupby_window
        if groupby_window:
            windowing_masks = (
                windowed_grouping_data[:, :, -1] - sorted_grouping_data[:, -1]
            ).abs() <= groupby_window // 2
            masks = masks * windowing_masks
        # correct for overflowing torch.roll
        for i, d in enumerate(offsets):
            if d < 0:  # successors not available at end
                masks[i, d:] = False
            elif d > 0:  # predecessors not available at start
                masks[i, :d] = False

        # mask and aggregate
        def _aggregate_vec(vec: torch.Tensor, fn: Callable):
            windowed_vec = torch.stack(
                [torch.roll(vec[sorted_idxs], d, dims=0) for d in offsets], dim=0
            )
            # hacky; requires aggregation that excludes nans
            windowed_vec[~masks] = torch.nan
            return fn(windowed_vec, dim=0, keepdim=True)

        if weight is not None:
            weight = weight.float()
        if normalize:
            norm_factor = _aggregate_vec(weight, fn=torch.nansum)
        for aggregate_field in aggregate_fields:
            aggr_data = self.data[aggregate_field]
            if weight is not None:
                aggr_data = aggr_data * weight
            aggr_data = _aggregate_vec(aggr_data, fn=aggregate_fn)
            if normalize:
                aggr_data = aggr_data / norm_factor
            grouped_data[aggregate_field][sorted_idxs] = aggr_data
    ...
    def groupby_weighted(
        ...
        weight: torch.Tensor,
        groupby_window: int | None = None,
        aggregate_fields: list[str] | tuple[str] = ("xyz", "phot", "bg"),
        sanity_check: bool = False,
        _normalize: bool = True,
        _aggregate_fn: Callable = torch.nansum,
    ) -> "EmitterSet":
        """
        Groupby with weighted aggregation. Alias for groupby with sensible defaults.

        Args:
            groupby: field name(s) to group by
            weight: importance weight for aggregation (multiply aggregated fields
             by weight)
            groupby_window: window size for temporal context
            aggregate_fields: fields to aggregate with aggregate_fn, all others are
             copied as they are
            sanity_check: perform sanity check on subset, disabled by default for speed
            _normalize: perform normalization
            _aggregate_fn: function to aggregate aggregate_fields of emitters in the
             same group
    ...
    def collapse_on(self, on: str):
        """
        Collapses the emitterset on a specific attribute. It will simply take the first
        element of the collapsed attribute and discard the rest.
        This is useful after grouping.

        Args:
            on: attribute to collapse on
        ...
        def get_first(t, dim):
            unique, idx, counts = torch.unique(
                t, dim=dim, sorted=True, return_inverse=True, return_counts=True
            )
        ...
        if on not in self.data.keys():
            raise ValueError(f"Can not collapse on {on}.")

        ix = get_first(self.data[on], dim=0)
        return self[ix]

    def assign_prob_code(self, ratio: torch.Tensor, priors: torch.Tensor | None = None):
        """
        Assigns probabibilsitic code to EmitterSet based on expected photon ratios.
        Needs photon counts and respective uncertainties. Assumes gaussian uncertainties.

        Args:
            ratio: matrix of size N x M, where N is the number of photon channels
             and M is the number of codes. The ratios typically sum up to 1 accross
             the photon channels.
            priors: prior probability of each code. If None, uniform prior is assumed.
        """
        em = self.clone()
        em.assign_prob_code_(ratio, priors)
        return em

    def assign_prob_code_(
        self, ratio: torch.Tensor, priors: torch.Tensor | None = None
    ):
        self.prob_code = self._calc_prob_code(ratio, priors)

    def _calc_prob_code(self, ratio: torch.Tensor, priors: torch.Tensor | None = None):
        """
        Computes posteriors for code assignment.
        """

        batch_size, num_channels = self.phot.shape
        _, num_colors = ratio.shape

        # Normalizing the photon counts for each batch
        phot_tot = self.phot.sum(1, keepdims=True)
        phot_normed = self.phot / phot_tot

        # Preparing the covariance matrix for each batch
        covariance_matrices = (self.phot_sig / phot_tot) ** 2
        covariance_matrices = torch.diag_embed(covariance_matrices)

        # Vectorized calculation of likelihoods
        likelihoods = torch.zeros(batch_size, num_colors)
        for color_index in range(num_colors):
            expected_ratios = ratio[:, color_index]
            dist = torch.distributions.multivariate_normal.MultivariateNormal(
                expected_ratios.repeat(batch_size, 1), covariance_matrices
            )
            likelihoods[:, color_index] = torch.exp(dist.log_prob(phot_normed))

        priors = torch.ones(num_colors) / num_colors if priors is None else priors
        # Calculating unnormalized posteriors
        posteriors_batch = likelihoods * priors

        # Normalizing the posteriors
        posteriors_batch /= posteriors_batch.sum(1, keepdim=True)

        return posteriors_batch
    ...
    def filter_by_sigma(
        ...
        tot_sig = self._calc_sigma_weighted_total(self.xyz_sig_nm, is_3d)

        max_s = np.percentile(tot_sig.cpu().numpy(), fraction * 100.0)
        if return_low:
    ...
    def filter_n(self, n: float, field: str = "xyz_sig_tot_nm", highest: bool = False):
        """
        Filter by probability, sigma, or photons. Returns EmitterSet.

        Args:
            n: number of emitters remaining after filtering.
            field: attribute/property of emitterset to filter by.
            highest: if True, return the n emitters with the highest values;
                else, return the n emitters with the lowest values.
        """
        # stable ensures consistent results for equal values
        sorted_ix = torch.argsort(getattr(self, field), stable=True)
        # not using descending because the order between self.filter_n(n, highest=False) and
        # self.filter_n(len(self) - n, highest=True) is not inverted, so overlaps of
        # self.filter_n(n, highest=False) and self.filter_n(len(self) - n, highest=True)
        # would be possible
        if highest:
            filt = sorted_ix[-n:]
        else:
    ...
    def split_in_frames(self, ix_low: int = 0, ix_up: int = None) -> list:
        ...
        This is useful if you want to process the emitters in a frame-wise manner, and
        about one magnitude faster than iterating via `.iframe` or `[em.frame_ix = i]`
        individually.
    ...
    def populate_crlb(self, psf, inversion: Optional[Callable] = None, **kwargs):
        """
        Populate the CRLB values by the PSF function.

        Args:
            psf: Point Spread function with CRLB implementation
            inversion: custom inversion method
            **kwargs: additional arguments to be parsed to the CRLB method

        Returns: EmitterSet object
    ...
    def populate_crlb_(self, psf, inversion: Optional[Callable] = None, **kwargs):
        ...
        """
        crlb, _ = psf.crlb(self.xyz, self.phot, self.bg, inversion=inversion, **kwargs)
        self.xyz_cr = crlb[:, :3]
        self.phot_cr = crlb[:, 3]
        self.bg_cr = crlb[:, 4]
    ...
    def repeat(self, repeats: Union[int, torch.Tensor], step_frames: bool):
        ...
        if step_frames:
            ...
            cum_count = gen_utils.cum_count_per_group(pseudo_id)

            em.frame_ix += cum_count.to(self.device)
    ...
    def _infer_code_length(attr_coord: dict, attr_generic: dict) -> int:
        ...
        coord_ch = {xyz.size(1) for xyz in attr_coord.values()}
        generic_ch = {generic.size(-1) for generic in attr_generic.values()}
        n_repeats = coord_ch | generic_ch

        if len(n_repeats) != 1:
            raise ValueError(
                f"Inconsistent number of channels but channels are present."
                f" got {n_repeats}, "
                f" coord attr is {coord_ch}, (coord keys {attr_coord.keys()})"
                f" generic is {generic_ch}, (generic keys {attr_generic.keys()})"
            )
    ...
    def infer_code(self) -> torch.Tensor:
        ...
        code = torch.arange(n_repeats, device=self.device)
        code = code.unsqueeze(0).repeat(len(self), 1)
        return code
...
def factory(
    ...
    **kwargs,
    ...
    Args:
        ...
        sanity_check: perform sanity check on emitter set
    ...
    essentials = {
        "xyz": torch.rand(n, 3, device=device) * extent,
        "phot": torch.ones(n, device=device),
        "frame_ix": torch.zeros(n, device=device),
    }
    ...
    return EmitterSet(**essentials, **kwargs, sanity_check=sanity_check)


@deprecated(reason="deprecated in favor of factory", version="0.11", action="error")
