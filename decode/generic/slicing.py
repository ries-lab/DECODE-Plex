...
def split_sliceable(x, x_ix: torch.LongTensor, ix_low: int, ix_high: int) -> list:
    ...
    if x_ix.numel() >= 1 and not dtype.is_integer(x_ix):
        raise TypeError("Index must be subtype of integer.")

    if len(x_ix) != len(x):
        raise ValueError(
            "Index and sliceable are not of same length (along first index)."
        )
    ...
    picker = torch.arange(ix_low, max(ix_high + 1, ix_low), device=x_ix.device)
    ix_sort = torch.searchsorted(x_ix, picker)
...
def ix_split(ix: torch.Tensor, ix_min: int, ix_max: int) -> list:
    """
    Splits an index rather than a sliceable (compared to `split_sliceable`).
...
@deprecated(version="0.11.0", reason="never used.")
...
class _LinearGetitemMixin(ABC):
    ...
    def __getitem__(self, item: Union[int, slice, tuple[int, slice]]):
        ...
        if isinstance(item[0], type(...)):
            raise NotImplementedError(f"Ellipsis not yet supported in first dimension.")
        ...
        if isinstance(item[0], slice):
            item[0] = list(range(len(self)))[
                item[0]
            ]  # convert slice to equivalent list

        # # catch list of lists in first dim
        if any([isinstance(i, list) for i in item[0]]):
            raise NotImplementedError(
                f"List of lists indexing in first item not supported. Got {item[0]}"
            )

        non_batch_getter = (slice(None), *item[1:])  # helper for correct reduction
        return self._collect_batch([self._get_element(i) for i in item[0]])[
            non_batch_getter
        ]
